...
namespace Com.MyCompany.MyGame.FileIO
{
    public static class DBIO
    {
        ...
        private static void Write(List<UnitStat> data)
        {
            bool CreateTable = false;
            string filePath = @"Data Source=" + FilePaths.DataPath + "/" + fileName;
        ...
        /// <returns></returns>
        public static UnitStat Read(string unitCode)
        {
            string filePath = @"" + FilePaths.DataPath + "/" + fileName;
            //파일이 없다면 만든다.
    ...
    public static class DataIO
    {
        ...
        private static void Write()
        {
            ...
            data.AppendLine("TestStage,9999.99,0.000");
            data.AppendLine("SecondStage,9999.99,0.000");

            File.WriteAllText(filePath, data.ToString());
        }

        public static void Write(string stageName, float gameTime, float gameScore)
        {
            string filePath = FilePaths.DataPath + "/" + fileName;
            ...
            for (int i = 0; i < allData.Length; i++)
            {
                if (allData[i].Contains(stageName))
                    allData[i] = string.Format("{0},{1},{2}", stageName, gameTime, gameScore);

                File.WriteAllText(filePath, allData[i]);
            }
        }

        public static GameResult Read(string stageName)
        {
            string filePath = FilePaths.DataPath + "/" + fileName;
            //파일이 없다면 만든다
            if (!File.Exists(filePath)) Write();
